...
function table2(ctx, pen) {
  ...
  const options = pen.calculative.canvas.store.options;
  pen.color = pen.color || store.styles.color;
  pen.textColor = pen.textColor || pen.color || store.styles.textColor;
  pen.activeColor = pen.activeColor || store.styles.activeColor;
  pen.hoverColor = pen.hoverColor || store.styles.hoverColor;
  pen.activeBackground = pen.activeBackground || store.styles.activeBackground;
  pen.hoverBackground = pen.hoverBackground || store.styles.hoverBackground;
...
function radio(ctx, pen) {
  ...
  if (!pen.onAdd) {
    ...
    pen.onBeforeValue = beforeValue4;
  }
  let x = pen.calculative.worldRect.x;
  let y = pen.calculative.worldRect.y;
  let h = pen.calculative.worldRect.height;
...
function beforeValue4(pen, value) {
  pen.calculative.flag = false;
  if (value.options !== void 0 || value.direction !== void 0 || value.optionInterval !== void 0 || value.optionHeight !== void 0) {
...
function formatTime(pen) {
  ...
  const fn = new Function("year", "month", "day", "week", "hours", "minutes", "seconds", pen.timeFormat ? `return ${pen.timeFormat}` : "return `${year}:${month}:${day} ${hours}:${minutes}:${seconds} 星期${week}`");
