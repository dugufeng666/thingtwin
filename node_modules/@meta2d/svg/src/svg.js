...
export function parseSvg(svg) {
    ...
    svgs.forEach((svg) => {
        ...
        const children = svg.svg;
        const replaceMap = {}; // 暂时存储改动，避免对遍历产生影响
        ...
        const combinePens = transformCombines(selfProperty, children);
...
function transformCombines(selfProperty, children, svg) {
    const pens = [];
...
function transformPath(path, pen) {
    ...
    const width = rect.width / allRect.width <= 1 ? rect.width / allRect.width : 1;
    const height = rect.height / allRect.height <= 1 ? rect.height / allRect.height : 1;
    const pathPen = {
        ...pen,
        name: 'svgPath',
...
function transformNormalShape(childProperty, parentProperty, parentId) {
    ...
    if (finalProperty.fill === 'none') {
        ...
        const id = finalProperty.fill.replace('url(#', '').replace(')', '');
        if (gradientMap[id]) {
            ...
            if (gradientColor && !gradientColor.color) {
                // 颜色不存在，则查找父级
                gradientColor = linearGradient.find((item) => gradientColor.from === `#${item.id}`);
            }
...
function transformText(childProperty, textContent, pen) {
    ...
    return {
        ...
        textAlign: 'left',
        text,
        x: (parseFloat(childProperty.x) + (offsetX || 0) - allRect.x + pen.x) /
            allRect.width,
        y: (parseFloat(childProperty.y) +
            (offsetY || 0) -
            allRect.y +
            pen.y -
            height) /
